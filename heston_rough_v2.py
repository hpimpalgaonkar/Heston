...
class base_heston():
    ...
    nu = Multiplier in the second integral
    Lambda = Parameter from the Rough Heston Model
    """
    
    def __init__(self, mu=0.1, rho=0, sigma=0.2, theta=-0.1, kappa=0.1, K = 100, alpha=0.6,lamb = 0.1, nu = 0.1):
        self.mu = mu
        self.K = K
        if (np.abs(rho)>1):
    ...
    def integrate(self, S0, v0, N, T=1):
        ...
        for t in range(0,N-1):
            ...
            if(t>=1):
                ...
                v[t+1] = np.maximum(v0 + 1/gamma(self.alpha)*simps(((t+1)*dt-x)**(self.alpha-1)*self.lamb*(self.theta-vinterp),x) + 1/gamma(self.alpha)*simps(((t+1)*dt-x)**(self.alpha-1)*self.lamb*self.nu * vsqinterp * dt_sq * W_v[t],x),0)
            else:
                v[t+1] = v0
...
K = 80                                                       # Strike price
v0 = 0.0392                                                # initial volatility
r = 0.3                                                    # Riskless Rate of return 
mu = 0; rho = -0.681; kappa = 6.21; theta = 0.3156; sigma = 0.6; nu = 0.331    #values for different 
#assert(2*kappa * theta <= sigma**2)                            # Condition from the paper is asserted, a method to check if the parameters are correct
num_sims = 100                                            #Monte Carlo simulation paths
call_payoff = 0.0                                             #Payoff for call option
put_payoff = 0.0                                              #Payoff for put option

np.random.seed(seed=1111)
#Initialize the class
Hest = base_heston(mu=mu, rho=rho, sigma=sigma, theta=theta, kappa=kappa, K=K, nu = nu)
