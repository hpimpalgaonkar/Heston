#=============================================================================================================================
#   Heston Model - To find the correct price of the options
...
class base_heston():
    ...
    def __init__(self, mu=0.1, rho=0, sigma=0.2, theta=-0.1, kappa=0.1, K = 100):
        self.mu = mu
        self.K = K
        if (np.abs(rho)>1):
    ...
    def integrate(self, S0, v0, N, T=1):
        ...
        for t in range(0,N-1):
            v_sq = np.sqrt(v[t])
            v[t+1] = np.maximum(v[t] + self.kappa*(self.theta - v[t])*dt + self.sigma * v_sq * dt_sq * W_v[t], 0.0)
            #X[t+1] = X[t] + (self.mu - 0.5*v[t])*dt + v_sq * dt_sq * W_S[t]        #incorrect
            #X[t+1] = X[t] + (self.mu)*X[t]*dt + v_sq * dt_sq *X[t]* W_S[t]         #from paper
            X[t+1] = X[t] * np.exp((self.mu - 0.5*v[t])*dt + v_sq * dt_sq * W_S[t] ) #same as paper
...
num_sims = 8000                                             #Monte Carlo simulation paths
